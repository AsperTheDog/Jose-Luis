...
class MiningSystemCog(commands.Cog):
    ...
    async def drink(self, interaction: discord.Interaction, modo: Optional[str] = "single"):
        ...
        await self.bot.db.mining_apply_refill(user_id, new_energy, new_choskris)

        refill_alert = "" if cost_perc == 0 else f" (+{cost_perc}% por {refills + 1} bebidas hoy)"
        await self.bot.global_stats.register_drink_action(interaction.user.id, total_cost)
        await interaction.response.send_message(
    ...
    async def equip(self, interaction: discord.Interaction, indice: Optional[int] = None):
        ...
        await self.bot.db.mining_equip_pickaxe(user_id, target_db_id)
    ...
    async def craft(self, interaction: discord.Interaction, receta_id: Optional[str] = None):
        ...
        inventory = await self.bot.db.mining_get_user_inventory(user_id)
        ...
        await self.bot.db.mining_craft_item(user_id, recipe["ingredients"], result_id, recipe["type"], max_dur)

        await self.bot.global_stats.register_item_crafted(interaction.user.id)
