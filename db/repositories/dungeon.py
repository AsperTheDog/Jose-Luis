...
class DungeonRepository(BaseRepository):
    ...
    async def ensure_user(self, user_id: int) -> None:
        await self._db.execute(
            "INSERT OR IGNORE INTO dungeon_users (user_id, potions) VALUES (?, 3)", (user_id,)
        )
    ...
    async def update_user(self, user_id: int, **fields) -> None:
        ...
        await self._db.execute(
            f"UPDATE dungeon_users SET {assignments}, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?",
    ...
    async def add_gold(self, user_id: int, amount: int) -> int:
        await self.ensure_user(user_id)
        await self._db.execute(
            "UPDATE dungeon_users SET gold = MAX(0, gold + ?), updated_at = CURRENT_TIMESTAMP WHERE user_id = ?",
            (int(amount), user_id),
        )
        await self._db.commit()
    ...
    async def spend_gold(self, user_id: int, amount: int) -> bool:
        ...
        cursor = await self._db.execute(
            "UPDATE dungeon_users SET gold = gold - ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ? AND gold >= ?",
    ...
    async def add_dust(self, user_id: int, amount: int) -> None:
        await self.ensure_user(user_id)
        await self._db.execute(
            "UPDATE dungeon_users SET dust = MAX(0, dust + ?), updated_at = CURRENT_TIMESTAMP WHERE user_id = ?",
            (int(amount), user_id),
        )
        await self._db.commit()
    ...
    async def add_boss_coins(self, user_id: int, amount: int) -> None:
        await self.ensure_user(user_id)
        await self._db.execute(
            "UPDATE dungeon_users SET boss_coins = MAX(0, boss_coins + ?), updated_at = CURRENT_TIMESTAMP WHERE user_id = ?",
            (int(amount), user_id),
        )
        await self._db.commit()
    ...
    async def spend_boss_coins(self, user_id: int, amount: int) -> bool:
        await self.ensure_user(user_id)
        cursor = await self._db.execute(
            "UPDATE dungeon_users SET boss_coins = boss_coins - ?, updated_at = CURRENT_TIMESTAMP "
            "WHERE user_id = ? AND boss_coins >= ?",
            (int(amount), user_id, int(amount)),
        )
        await self._db.commit()
        return cursor.rowcount > 0
    ...
    async def get_inventory(self, user_id: int) -> list[dict]:
        ...
        async with self._db.execute(
            "SELECT * FROM dungeon_inventory WHERE user_id = ? ORDER BY is_equipped DESC, acquired_at DESC",
            (user_id,),
        ) as cursor:
    ...
    async def equip_item(self, user_id: int, item_uid: str) -> bool:
        async with self._db.execute(
            "SELECT slot FROM dungeon_inventory WHERE user_id = ? AND item_uid = ?", (user_id, item_uid)
        ) as cursor:
            row = await cursor.fetchone()
        if not row:
            return False
        ...
        await self._db.execute(
            "UPDATE dungeon_inventory SET is_equipped = 1 WHERE user_id = ? AND item_uid = ?", (user_id, item_uid)
        )
        await self._db.commit()
        return True
    ...
    async def delete_item(self, user_id: int, item_uid: str) -> bool:
        cursor = await self._db.execute(
            "DELETE FROM dungeon_inventory WHERE user_id = ? AND item_uid = ?", (user_id, item_uid)
        )
        await self._db.commit()
        return cursor.rowcount > 0
    ...
    async def get_item(self, user_id: int, item_uid: str) -> Optional[dict]:
        async with self._db.execute(
            "SELECT * FROM dungeon_inventory WHERE user_id = ? AND item_uid = ?", (user_id, item_uid)
        ) as cursor:
            row = await cursor.fetchone()
        if not row:
            return None
    ...
    async def set_item_sockets(self, user_id: int, item_uid: str, sockets: list[dict]) -> None:
        await self._db.execute(
            "UPDATE dungeon_inventory SET sockets = ? WHERE user_id = ? AND item_uid = ?",
            (_dumps(sockets), user_id, item_uid),
        )
        await self._db.commit()
    ...
    async def get_mutations(self, user_id: int) -> dict[str, int]:
        async with self._db.execute(
            "SELECT mutation_id, level FROM dungeon_mutations WHERE user_id = ?", (user_id,)
        ) as cursor:
            rows = await cursor.fetchall()
    ...
    async def set_mutation(self, user_id: int, mutation_id: str, level: int) -> None:
        await self._db.execute(
            "INSERT INTO dungeon_mutations (user_id, mutation_id, level) VALUES (?, ?, ?) "
            "ON CONFLICT(user_id, mutation_id) DO UPDATE SET level = excluded.level",
            (user_id, mutation_id, int(level)),
        )
        await self._db.commit()
    ...
    async def get_echoes(self, user_id: int) -> dict[str, int]:
        async with self._db.execute(
            "SELECT echo_id, level FROM dungeon_echoes WHERE user_id = ?", (user_id,)
        ) as cursor:
            rows = await cursor.fetchall()
        return {row[0]: int(row[1]) for row in rows}
    ...
    async def set_echo(self, user_id: int, echo_id: str, level: int) -> None:
        await self._db.execute(
            "INSERT INTO dungeon_echoes (user_id, echo_id, level) VALUES (?, ?, ?) "
            "ON CONFLICT(user_id, echo_id) DO UPDATE SET level = excluded.level",
            (user_id, echo_id, int(level)),
        )
        await self._db.commit()
    ...
    async def save_fight(self, user_id: int, state: dict) -> None:
        await self._db.execute(
            "INSERT INTO dungeon_active_fight (user_id, state, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP) "
            "ON CONFLICT(user_id) DO UPDATE SET state = excluded.state, updated_at = CURRENT_TIMESTAMP",
            (user_id, _dumps(state)),
        )
        await self._db.commit()
    ...
    async def get_cosmetics(self, user_id: int) -> dict[str, int]:
        async with self._db.execute(
            "SELECT cosmetic_id, equipped FROM dungeon_cosmetics WHERE user_id = ?", (user_id,)
        ) as cursor:
            rows = await cursor.fetchall()
        return {row[0]: int(row[1]) for row in rows}
    ...
    async def add_cosmetic(self, user_id: int, cosmetic_id: str) -> bool:
        cursor = await self._db.execute(
            "INSERT OR IGNORE INTO dungeon_cosmetics (user_id, cosmetic_id, equipped) VALUES (?, ?, 0)",
            (user_id, cosmetic_id),
        )
        await self._db.commit()
        return cursor.rowcount > 0
    ...
    async def equip_cosmetic(self, user_id: int, cosmetic_id: str, group: list[str]) -> None:
        ...
        await self._db.execute(
            f"UPDATE dungeon_cosmetics SET equipped = 0 WHERE user_id = ? AND cosmetic_id IN ({placeholders})",
            (user_id, *group),
        )
        await self._db.execute(
            "UPDATE dungeon_cosmetics SET equipped = 1 WHERE user_id = ? AND cosmetic_id = ?",
            (user_id, cosmetic_id),
        )
        await self._db.commit()
    ...
    async def add_log(self, user_id: int, kind: str, message: str) -> None:
        await self._db.execute(
            "INSERT INTO dungeon_log (user_id, kind, message) VALUES (?, ?, ?)", (user_id, kind, message)
        )
        await self._db.commit()

    async def recent_log(self, user_id: int, limit: int = 15) -> list[dict]:
        async with self._db.execute(
            "SELECT kind, message, created_at FROM dungeon_log WHERE user_id = ? ORDER BY id DESC LIMIT ?",
            (user_id, int(limit)),
        ) as cursor:
            rows = await cursor.fetchall()
    ...
    async def set_automation(self, user_id: int, **fields) -> None:
        await self._db.execute(
            "INSERT OR IGNORE INTO dungeon_automation (user_id) VALUES (?)", (user_id,)
        )
        ...
        assignments = ", ".join(f"{name} = ?" for name in updates)
        await self._db.execute(
            f"UPDATE dungeon_automation SET {assignments} WHERE user_id = ?", (*updates.values(), user_id)
        )
        await self._db.commit()
